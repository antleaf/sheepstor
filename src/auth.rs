...
pub fn validate_github_secret3(secret: &str, headers: HeaderMap, payload: String) -> Result<(), Box<dyn std::error::Error>> {
                ...
                if let Some(sig_sep) = signature.strip_prefix("sha256=") {
                    match hex::decode(sig_sep) {
                        Ok(decoded) => match mac.verify_slice(&decoded) {
                            Ok(()) => Ok(()),
                            Err(e) => Err(Box::new(e)),
                            // Err(_) => Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidData,"Failed to verify signature"))),
                        },
                        Err(e) => Err(Box::new(e)),
                    }
                } else {
